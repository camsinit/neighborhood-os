...
-- Step 2: Drop existing function with CASCADE to handle dependencies
...
-- Step 4: Recreate the dependent policies that were dropped
CREATE POLICY "Super admins can manage all events" ON public.events
FOR ALL
TO authenticated
USING (check_user_role(auth.uid(), 'super_admin'::user_role));

CREATE POLICY "Super admins can manage all safety updates" ON public.safety_updates
FOR ALL
TO authenticated
USING (check_user_role(auth.uid(), 'super_admin'::user_role));

CREATE POLICY "Super admins can manage all support requests" ON public.support_requests
FOR ALL
TO authenticated
USING (check_user_role(auth.uid(), 'super_admin'::user_role));

CREATE POLICY "Allow admin read to waitlist" ON public.waitlist
FOR SELECT
TO authenticated
USING (check_user_role(auth.uid(), 'admin'::user_role) OR check_user_role(auth.uid(), 'super_admin'::user_role));

CREATE POLICY "Allow admin update to waitlist" ON public.waitlist
FOR UPDATE
TO authenticated
USING (check_user_role(auth.uid(), 'admin'::user_role) OR check_user_role(auth.uid(), 'super_admin'::user_role));

CREATE POLICY "Allow admin delete from waitlist" ON public.waitlist
FOR DELETE
TO authenticated
USING (check_user_role(auth.uid(), 'admin'::user_role) OR check_user_role(auth.uid(), 'super_admin'::user_role));

-- Step 5: Create secure role management functions
...
-- Step 6: Create secure role removal function
...
-- Step 7: Create new secure RLS policies for user_roles (admin-only access)
