-- Phase 1B: Create neighborhood roles table and supporting functions

-- Create neighborhood_roles table for neighborhood-scoped permissions
...
-- Enable RLS on neighborhood_roles table
ALTER TABLE neighborhood_roles ENABLE ROW LEVEL SECURITY;

-- Create RLS policies for neighborhood_roles
...
-- Create function to automatically assign admin role to neighborhood creators
...
-- Create trigger to automatically assign admin role when neighborhood is created
...
-- Create function to get user's role in a specific neighborhood
...
-- Migrate existing neighborhood creators to have admin role
INSERT INTO neighborhood_roles (user_id, neighborhood_id, role, assigned_by)
SELECT 
...
-- Update existing user roles to use 'neighbor' instead of 'user' where appropriate
UPDATE user_roles 
...
-- Add updated_at trigger for neighborhood_roles
