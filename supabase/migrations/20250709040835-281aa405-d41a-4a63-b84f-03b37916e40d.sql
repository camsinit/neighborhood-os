...
-- Drop inappropriate goods notification triggers
...
-- Log the changes
RAISE LOG 'Notification system updated to only send personally relevant notifications';
RAISE LOG 'Removed: goods notifications, general neighbor join notifications';
RAISE LOG 'Modified: safety notifications (emergency/suspicious only), skills notifications (targeted matches only)';
RAISE LOG 'Activity feed functionality preserved for all neighborhood events';