...
/**
 * Custom hook for community update actions
 * 
 * This hook centralizes all the actions related to community updates like
...
/**
 * Hook for managing community update actions and state
...
export const useSafetyUpdateActions = () => {
  ...
  /**
   * Handler for deleting community updates
   * Confirms with the user and processes the deletion
   */
  const handleDeleteSafetyUpdate = async (updateId: string) => {
    if (!user) return;

    if (window.confirm("Are you sure you want to delete this community update?")) {
      try {
        ...
        if (error) {
          logger.error("Error deleting community update:", error);
          toast.error("Failed to delete community update");
          return;
        }

        // Show success message and refresh data
        toast.success("Community update deleted successfully");
