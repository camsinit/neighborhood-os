...
export const UserDirectory = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data: isAdminResult
      } = await supabase.rpc('check_user_role', {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data: profiles,
        error: profilesError
      } = await supabase.from('profiles').select('id, display_name, avatar_url, address');
            ...
            <CardContent className="p-6">
              <div className="flex flex-col items-center space-y-4">
                ...
                <div className="text-center">
                  ...
                  {user.profiles?.address && <div className="flex items-center justify-center mt-2 text-sm text-gray-600">
                      <MapPin className="h-4 w-4 mr-1" />
                      <span>{user.profiles.address}</span>
                    </div>}
                </div>
                
                {isAdmin && <div className="flex items-center space-x-2 mt-4">
                    <Shield className="h-4 w-4 text-gray-400" />
                    <Select value={user.roles[0] || 'user'} onValueChange={role => {
