...
const ImageCarousel = ({
  ...
  return <div className="relative aspect-w-16 aspect-h-9 mb-4">
      {/* Current visible image */}
      <img src={images[currentImageIndex]} alt={`${title} - Image ${currentImageIndex + 1}`} className="object-cover rounded-md h-48 w-full" />
      ...
      {images.length > 1 && <>
          ...
          <button onClick={showPreviousImage} className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/30 hover:bg-black/50 text-white rounded-full p-1 transition-colors" aria-label="Previous image">
            <ChevronLeft className="h-5 w-5" />
          </button>
          
          {/* Next image button */}
          <button onClick={showNextImage} className="absolute right-2 top-1/2 -translate-y-1/2 bg-black/30 hover:bg-black/50 text-white rounded-full p-1 transition-colors" aria-label="Next image">
...
const AvailableItemsSection = ({
  ...
  if (goodsItems.length === 0) {
    return <div className="max-w-4xl mx-auto mt-8">
        <Button variant="outline" onClick={onNewOffer} className="w-full p-8 h-auto border-2 border-dashed border-gray-300 hover:border-gray-400 flex flex-col items-center gap-4 py-[25px] my-[21px]">
  ...
  return <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      {goodsItems.map(request => <div key={request.id} className="bg-white rounded-lg shadow-sm border p-4 hover:shadow-md transition-shadow cursor-pointer relative group">
          {/* Delete button - only shown for the creator when hovering */}
          {currentUser && currentUser.id === request.user_id && onDeleteItem && <Button variant="destructive" size="icon" className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity z-10 h-8 w-8" onClick={e => {
        e.stopPropagation();
        e.preventDefault();
        onDeleteItem(request);
      }} disabled={isDeletingItem} aria-label="Delete item">
          ...
          <div onClick={() => onRequestSelect(request)}>
            ...
            {request.image_url && <div className="aspect-w-16 aspect-h-9 mb-4">
                <img src={request.image_url} alt={request.title} className="object-cover rounded-md h-48 w-full" />
              </div>}
            
            {/* Case 2: If there are images in the images array, use the carousel */}
            {request.images && request.images.length > 0 && !request.image_url && <ImageCarousel images={request.images} title={request.title} />}
          ...
          <div className="absolute top-2 right-12">
            <ArchiveButton requestId={request.id} tableName="goods_exchange" onArchiveComplete={onRefetch} />
          </div>
        </div>)}
    </div>;
};
