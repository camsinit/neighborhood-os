...
/**
 * InviteDialogContent Component
 * 
 * Core content for the invitation functionality, extracted from InviteDialog
 * for better separation of concerns.
 */
const InviteDialogContent = ({ onClose }: { onClose: () => void }) => {
  // State for the email input and link generation process
  const [email, setEmail] = useState("");
  const [isGeneratingLink, setIsGeneratingLink] = useState(false);
  // State to detect stuck loading state
  ...
  useEffect(() => {
    // Only start timer if we're loading
    if (isLoading) {
  ...
  }, [user, currentNeighborhood, isLoading, error, isGeneratingLink, loadingTooLong]);
  ...
  const isStuckLoading = isLoading && loadingTooLong;
