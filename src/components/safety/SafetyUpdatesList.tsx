...
const SafetyUpdatesList = ({
   ...
   */
  const renderEmptyState = () => <div className="bg-white rounded-lg shadow-sm p-8 text-center">
      ...
      <div className="flex items-center gap-4 mb-6 px-0 py-0">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="outline" size="sm" className="text-gray-600 border-gray-200 bg-white">
          ...
          <DropdownMenuContent align="end" className="bg-white">
            {categories.map(category => <DropdownMenuItem key={category.value} onClick={() => setSelectedCategory(category)} className="cursor-pointer">
        ...
        <Button onClick={onAddUpdate} className="bg-red-500 hover:bg-red-600 text-white ml-auto">
          <Shield className="w-4 h-4 mr-2" />
          New Update
        </Button>
      </div>
      <div className="space-y-6">
        {isLoading ? renderSkeleton() : updates?.length > 0 ? updates.map(update => <SafetyUpdateCard key={update.id} update={update} onClick={() => onUpdateClick(update)} />) : renderEmptyState()}
      </div>
    </div>;
};
