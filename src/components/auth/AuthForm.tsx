...
const AuthForm = () => {
  ...
  useEffect(() => {
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event: 'INITIAL_SESSION' | 'SIGNED_IN' | 'SIGNED_OUT' | 'USER_UPDATED' | 'USER_DELETED' | 'PASSWORD_RECOVERY' | 'TOKEN_REFRESHED' | 'SIGNED_UP', session) => {
      if (event === 'SIGNED_UP') {
